        ...
        <div class="mb-2">
            <form action="{{ route('post.comment', ['post' => $post->id]) }}" method="post">
                ...
                <a href="/p/{{ Auth::user()->username }}" class="flex mb-2 items-center w-fit"><img
                        ...
                        class="w-8 h-8 shadow-md rounded-full flex-shrink-0 mr-2 bg-white sticky top-2" alt=""> <span
                        class="duration-300 font-bold hover:text-purple-700"> {{ '@' . Auth::user()->username }} </span>
                </a>
